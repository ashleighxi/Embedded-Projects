...
static volatile int16_t lastVoltage;
...
static void adc_handler(ADC_t *adc, uint8_t ch_mask, adc_result_t result)
{
	...
	snprintf(out_str, OUTPUT_STR_SIZE, "Voltage: %4d V", result);
	gfx_mono_draw_string(out_str, 0, 0, &sysfont);
	if (result == -1) {
		gfx_mono_draw_filled_rect(0, 10, 128, 5, GFX_PIXEL_CLR);
	} else {
		gfx_mono_draw_filled_rect(0, 10, result, 5, GFX_PIXEL_SET);
		gfx_mono_draw_filled_rect(result, 10, 128 - result, 5, GFX_PIXEL_CLR);
	}
